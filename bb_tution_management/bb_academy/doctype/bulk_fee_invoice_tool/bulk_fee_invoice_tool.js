...
frappe.ui.form.on("Bulk Fee Invoice Tool", {
	generate_invoices(frm) {
		if (!frm.doc.academic_year || !frm.doc.fee_month || !frm.doc.fee_year) {
			frappe.msgprint(__("Please select Academic Year, Fee Month, and Fee Year."));
			return;
		}

		frappe.confirm(
			__("Are you sure you want to generate Bulk Invoices for {0} {1}?", [frm.doc.fee_month, frm.doc.fee_year]),
			() => {
				frappe.call({
